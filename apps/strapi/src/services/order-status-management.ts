...
export default {
  ...
  async updateOrderStatus(request: StatusUpdateRequest) {
    try {
      ...
      const statusRecord = await strapi.documents('api::order-status.order-status').create({
        data: {
          order: orderId,
  ...
  async sendStatusNotification(order: any, status: string, reason?: string) {
    try {
      ...
      const statusRecord = await strapi.documents('api::order-status.order-status').findFirst({
        filters: { order: {documentId: order.documentId}, status: status as any }
      });

      // Update the status record
      await strapi.documents('api::order-status.order-status').update({
  ...
  async verifyPayment(orderId: string) {
    ...
    if (order.manualPayment) {
      ...
      const payment = await strapi.documents('api::manual-payment.manual-payment').findOne({
        documentId: order.manualPayment.documentId
      });

...
export default {
  ...
  async createTrackingRecord(request: TrackingRequest) {
    try {
      ...
      const existingTracking = await strapi.documents('api::order-tracking.order-tracking').findFirst({
      ...
      const trackingRecord = await strapi.documents('api::order-tracking.order-tracking').create({
        data: trackingData,
        populate: ['order']
      });
  ...
  async updateTrackingStatus(trackingId: string, update: TrackingUpdate) {
    try {
      const tracking = await strapi.documents('api::order-tracking.order-tracking').findOne({
      ...
      const updatedTracking = await strapi.documents('api::order-tracking.order-tracking').update({
        documentId: trackingId,
  ...
  async getTrackingInfo(trackingNumber: string, carrier?: string) {
    try {
      ...
      const tracking = await strapi.documents('api::order-tracking.order-tracking').findFirst({
        filters,
  ...
  async fetchCarrierUpdates(trackingId: string) {
    try {
      ...
      await strapi.documents('api::order-tracking.order-tracking').update({
        documentId: trackingId,
        data: {
  ...
  async getTrackingStatistics() {
    try {
      const totalTrackings = await strapi.documents('api::order-tracking.order-tracking').count({});
      
      const statusCounts = await Promise.all([
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'pending' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'in_transit' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'out_for_delivery' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'delivered' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'failed' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'returned' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'lost' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { status: 'damaged' } as any })
      ]);

      const carrierCounts = await Promise.all([
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { carrier: 'fedex' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { carrier: 'ups' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { carrier: 'usps' } as any }),
        strapi.documents('api::order-tracking.order-tracking').count({ filters: { carrier: 'dhl' } as any })
      ]);
  ...
  async cleanupOldTrackingRecords(olderThanDays: number = 90) {
    try {
      ...
      const oldRecords = await strapi.documents('api::order-tracking.order-tracking').findMany({
        filters: {
      ...
      for (const record of oldRecords) {
        await strapi.documents('api::order-tracking.order-tracking').delete({

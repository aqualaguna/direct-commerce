...
export default factories.createCoreService('api::product.product', ({ strapi }) => ({
   ...
   */
  async applyCategoryFilter(filters: any, categoryId?: number, includeSubcategories: boolean = true) {
    if (!categoryId) {
      return filters;
    }
  ...
  async getSubcategories(categoryId: number): Promise<any[]> {
    try {
      const directChildren: any = await strapi.entityService.findMany('api::category.category', {
        filters: {
      ...
      let allSubcategories = Array.isArray(directChildren) ? directChildren : directChildren.data || [];

      // Recursively get subcategories
      for (const child of allSubcategories) {
        const grandChildren = await this.getSubcategories(child.id);
   ...
   * @returns Updated filters object
   */
  applyPriceFilter(filters: any, priceRange?: { min?: number; max?: number }) {
    if (!priceRange) {
      return filters;
    }
   ...
   * @returns Updated filters object
   */
  applyAttributeFilters(filters: any, attributes?: Record<string, any>) {
   ...
   * @returns Updated filters object
   */
  applyInventoryFilter(filters: any, inStockOnly: boolean = false, minStock?: number) {
    const updatedFilters = { ...filters };
   ...
   */
  validateFilterParams(filterParams: any) {
    const sanitized: any = {};
    ...
    if (filterParams.inStockOnly !== undefined) {
      sanitized.inStockOnly = filterParams.inStockOnly === 'true' || filterParams.inStockOnly === true;
    }
    ...
    const knownSystemFields = ['categoryId', 'minPrice', 'maxPrice', 'inStockOnly', 'minStock', 'q', 'page', 'pageSize', 'sortBy', 'sortOrder', 'includeInactive'];
    
    Object.entries(filterParams).forEach(([key, value]) => {
      if (!knownSystemFields.includes(key) && value !== undefined && value !== null && value !== '') {
   ...
   */
  async buildFilters(baseFilters: any, filterParams: any) {
    let filters = { ...baseFilters };

    // Apply category filtering
    if (filterParams.categoryId) {
      filters = await this.applyCategoryFilter(filters, filterParams.categoryId, true);
    }
    ...
    if (filterParams.inStockOnly || filterParams.minStock !== undefined) {
      filters = this.applyInventoryFilter(filters, filterParams.inStockOnly, filterParams.minStock);
    }
  ...
  async getAvailableFilters() {
    try {
      ...
      const priceStats = await strapi.db.query('api::product.product').findMany({
        select: ['price'],
        where: {
          publishedAt: { $notNull: true },
          status: 'active',
      ...
      const hierarchicalCategories = this.buildCategoryHierarchy(categoryData);
   ...
   */
  buildCategoryHierarchy(categories: any[]) {
    ...
    categories.forEach(category => {
      ...
      if (category.parent) {
        const parentNode = categoryMap.get(category.parent.id || category.parent);
        if (parentNode) {

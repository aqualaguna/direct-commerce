...
export default factories.createCoreService('api::product.product', ({ strapi }) => ({
   ...
   */
  async fullTextSearch(searchQuery: string, options: SearchOptions = {}) {
    const {
      page = 1,
      pageSize = 25,
      categoryId,
    ...
    try {
      // Base filters for published and active products
      ...
      let filters = await filterService.buildFilters(baseFilters, filterParams);
      ...
      const results: any = await strapi.entityService.findMany('api::product.product', {
        filters,
        sort,
      ...
      if (searchQuery && searchQuery.trim()) {
        ...
        const scoredResults = this.calculateRelevanceScores(products, searchQuery.trim());
        return {
          data: scoredResults,
          meta: results.meta || {
            pagination: {
   ...
   */
  calculateRelevanceScores(products: any[], searchQuery: string) {
  ...
  async getSearchSuggestions(partialQuery: string, limit: number = 10) {
    ...
    try {
      ...
      // Get product titles that start with or contain the query
...
export interface SearchOptions {
  ...
  sortBy?: 'relevance' | 'price_asc' | 'price_desc' | 'title' | 'newest' | 'oldest';
  sortOrder?: 'asc' | 'desc';

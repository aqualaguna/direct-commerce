...
export default {
  ...
  async recordHistoryEvent(event: HistoryEvent) {
    try {
      ...
      const historyRecord = await strapi.documents('api::order.order-history').create({
  ...
  async getOrderHistory(query: HistoryQuery) {
    try {
      ...
      const history = await strapi.documents('api::order.order-history').findMany({
        filters,
        sort: 'createdAt:desc',
  ...
  async getHistoryStatistics(orderId?: string, startDate?: Date, endDate?: Date) {
    try {
      ...
      const totalEvents = await strapi.documents('api::order.order-history').count(filters);

      const eventTypeCounts = await Promise.all([
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'order_created' }),
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'status_changed' }),
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'payment_updated' }),
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'shipping_updated' }),
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'address_changed' }),
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'notes_updated' }),
        strapi.documents('api::order.order-history').count({ ...filters, eventType: 'fraud_flag_raised' })
      ]);

      const priorityCounts = await Promise.all([
        strapi.documents('api::order.order-history').count({ ...filters, priority: 'low' }),
        strapi.documents('api::order.order-history').count({ ...filters, priority: 'normal' }),
        strapi.documents('api::order.order-history').count({ ...filters, priority: 'high' }),
        strapi.documents('api::order.order-history').count({ ...filters, priority: 'critical' })
      ]);

      const followUpCount = await strapi.documents('api::order.order-history').count({
        ...filters,
        requiresFollowUp: true
      });
  ...
  async cleanupOldHistory(olderThanDays: number = 365) {
    try {
      ...
      const oldRecords = await strapi.documents('api::order.order-history').findMany({
        filters: {
      ...
      for (const record of oldRecords) {
        await strapi.documents('api::order.order-history').delete({
